		...
		if [[ "${_uname}" == "Darwin" ]]; then
			...
			if command_exists -v curl; then
				...
				if command_exists -v dpkg; then
					...
					sudo dnf install $_args "step-cli-${_step_latest_version}-1.${_arch}.rpm"
				else
		...
		if [[ "${_uname}" == "Darwin" ]]; then
			...
			if command_exists -v curl; then
				...
				if command_exists -v dpkg; then
					...
					curl -sLO "https://dl.smallstep.com/gh-release/cli/docs-ca-install/v${_step_version}/step-cli_${_step_version}-1_${_arch}.deb"
					sudo dpkg -i "step-cli_${_step_version}-1_${_arch}.deb"
				elif command_exists -v dnf; then
					curl -sLO "https://dl.smallstep.com/gh-release/cli/docs-ca-install/v${_step_version}/step-cli-${_step_version}-1.${_arch}.rpm"
					sudo dnf install "step-cli-${_step_version}-1.${_arch}.rpm"
				else
					echo "Could not find a valid package manager"
					false
				fi
